...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && p === "/api/health") return json(res, readHealth());
    if (req.method === "GET" && p === "/api/accounts") {
      if (!auth(req, url, res)) return;
      return json(res, readAccountStatus());
    }
...
async function streamGenerate(req, res, body) {
  ...
  writeJob(jobId, { id: jobId, status: "processing", created_at: Date.now() });
  ...
  try {
    ...
    finishJob(jobId, { error: formatError(err) });
...
async function generate(body, onProgress) {
  ...
  const candidates = pickAccounts(MAX_TRIES);
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      report({ account_id: acc.account_id, name: acc.name, ok: true });
      ...
      report({ account_id: acc.account_id, name: acc.name, ok: false, kind, error: msg });
      if (kind === "policy") {
...
function readHealth() {
  ...
  return {
    ok: true, backend: "node", total: POOL.accounts.length,
